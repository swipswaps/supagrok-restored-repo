#!/usr/bin/env python3
# refind_style_tracker.py — PRF‑WEBCAM‑TRACKER‑2025‑05‑02
# Description: Webcam tracker with green dots around face, orange dots around eyes, and rEFInd-style buttons
...
# === rEFInd Style Button Class ===
class RefindButton:
    """Button with rEFInd/GRUB style"""
    
    def __init__(self, x, y, width, height, text, icon_text, action=None):
        self.x = x
        self.y = y
        self.width = width
        self.height = height
        ...
        self.hover = False
        self.dwell_time = 0
        self.dwell_threshold = 1.5  # seconds (longer for more stability)
        
        # rEFInd style colors
        self.bg_color = (30, 30, 30)
        self.border_color = (100, 100, 100)
        self.text_color = (220, 220, 220)
        self.hover_border_color = (0, 120, 215)
        self.progress_color = (0, 180, 0)
    ...
    def draw(self, frame, font):
        """Draw the button on the frame in rEFInd style"""
        import cv2
        ...
        # Draw border (thicker and highlighted when hovering)
        border_color = self.hover_border_color if self.hover else self.border_color
        border_thickness = 3 if self.hover else 2
        cv2.rectangle(frame, (self.x, self.y), (self.x + self.width, self.y + self.height), 
                     border_color, border_thickness)
        
        # Draw icon text (large centered character)
        icon_size = 1.5
        icon_text_size = cv2.getTextSize(self.icon_text, font, icon_size, 2)[0]
        icon_x = self.x + (self.width - icon_text_size[0]) // 2
        icon_y = self.y + self.height // 2 + 10
        cv2.putText(frame, self.icon_text, (icon_x, icon_y), font, icon_size, 
                   self.text_color, 2)
        
        # Draw button text (below icon)
        text_size = cv2.getTextSize(self.text, font, 0.6, 1)[0]
        text_x = self.x + (self.width - text_size[0]) // 2
        text_y = self.y + self.height - 15
        cv2.putText(frame, self.text, (text_x, text_y), font, 0.6, 
                   self.text_color, 1)
...
def main():
    """Main function"""
    print("🚀 Starting rEFInd Style Tracker")
    ...
    try:
        ...
        # Create window
        cv2.namedWindow("rEFInd Style Tracker", cv2.WINDOW_NORMAL)
        cv2.resizeWindow("rEFInd Style Tracker", frame_width, frame_height)
        ...
        # Create buttons in rEFInd/GRUB style
        buttons = []
        button_width = 100
        button_height = 100
        margin = 20
        ...
        while running:
            ...
            # Show the frame
            cv2.imshow("rEFInd Style Tracker", display_frame)
