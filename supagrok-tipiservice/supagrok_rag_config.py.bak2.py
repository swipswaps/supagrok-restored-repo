...
def find_dotenv_config() -> TypingOptional[Path]:
    ...
    cwd = Path.cwd()
    # If running from a script within the project dir, .env might be there or in parent.
    # If project dir is the CWD, then just CWD.
    paths_to_check = [
        cwd / ".env",
        cwd.parent / ".env",
        cwd.parent.parent / ".env" # In case scripts are run from a sub-sub-directory
    ]
...
# Use a specific .env filename if needed, e.g. .env_rag
# For this generic config, we'll stick to standard .env search logic.
# The master setup script might create a specifically named .env file.
if env_path_found_config:
    ...
    load_dotenv() # Tries to load from existing env vars if .env is not found


OPENAI_API_KEY_CONFIG: TypingOptional[str] = os.getenv("OPENAI_API_KEY")

# Define the default storage path for the LlamaIndex vector store
# Relative to where the CLI script is executed.
STORAGE_DIR_NAME_CONFIG: str = "supagrok_rag_storage_v2_final" 
STORAGE_DIR_CONFIG_PATH: Path = Path.cwd() / STORAGE_DIR_NAME_CONFIG

# Preferred languages for YouTube subtitles
...
def ensure_openai_api_key() -> None:
    ...
    # Ensure the key is available in the environment for LlamaIndex or other libraries
