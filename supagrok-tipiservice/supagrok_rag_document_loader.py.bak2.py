...
import supagrok_youtube_ingest_utils # Our custom module for YouTube
import supagrok_rag_config # Our custom config module

logger = logging.getLogger(__name__)

# Regex to identify YouTube URLs (made slightly more robust)
...
def _load_document_from_youtube(video_url: str) -> List[Document]:
    ...
    # Get preferred languages from our config module
    ...
    if subtitle_data and subtitle_data["text"] and subtitle_data["text"].strip():
        ...
        # Create more detailed metadata for the LlamaIndex Document
        metadata = {
            "source_type": "youtube_video_subtitles", # More specific
            ...
            "original_input_source": video_url, # Keep track of the exact input
            "content_length_chars": len(doc_text)
        }
        # Using video_url as doc_id for uniqueness and easy reference
        # This helps in identifying the document later if needed
...
def load_documents(input_source_str: str) -> List[Document]:
    ...
    # Existing file/directory loading logic
    ...
    try:
        if input_path.is_file():
            ...
            # To ensure metadata['file_name'] is correct, it's better to point it to the dir.
            documents = SimpleDirectoryReader(
                input_dir=str(input_path.parent), # Directory containing the file
                input_files=[input_path.name]    # Specific file to load
            ).load_data()
            ...
            reader = SimpleDirectoryReader(
                ...
                # Optionally add required_exts=[".txt", ".md", ".pdf"] to filter file types
                # LlamaIndex SimpleDirectoryReader handles many common types by default.
            )
            documents = reader.load_data()
            logger.info(f"Loaded {len(documents)} documents from directory: {input_path}")
        else: # Should not happen if input_path.exists() is true and it's not a URL
...
if __name__ == "__main__":
    logging.basicConfig(level=logging.INFO) # Set logging level for __main__ tests
    ...
    # Test 2: YouTube URL loading
    # Using the RAG strategy video URL
    ...
    # Ensure supagrok_youtube_ingest_utils.py and its dependencies (yt-dlp) are working
