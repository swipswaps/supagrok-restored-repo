...
import logging
import sys 
from pathlib import Path # For checking current working directory in error message

# Attempt to import local Supagrok RAG modules.
# The order can matter, especially `supagrok_rag_vector_store` which initializes LlamaIndex Settings.
try:
    import supagrok_rag_config as app_config # Use an alias to avoid conflict with 'config' name if used locally
    # Importing vector_store early ensures LlamaIndex Settings (LLM, Embed model) are configured.
    # If this fails (e.g., API key issue caught by app_config.ensure_openai_api_key), the CLI won't start.
...
except ValueError as ve_cli_config: # Specifically catch API key errors from LlamaIndex Settings init
    ...
    sys.exit(1)
except Exception as e_cli_critical_setup: # Catch any other unexpected critical error during module loading
    print(f"CRITICAL CLI Error during initial module loading phase: {e_cli_critical_setup}", file=sys.stderr)
    sys.exit(1)

# Configure logging for the CLI application itself
# Other modules might have their own loggers, this sets a base for CLI-specific messages.
logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(name)s - %(levelname)s - %(message)s')
logger = logging.getLogger("SupagrokRAG_CLI_App") # Specific logger for CLI operations
...
def handle_ingest_command(args: argparse.Namespace) -> None:
    ...
    try:
        # API key check is implicitly done by module imports (vector_store -> config),
        # but an explicit check here before intensive operations is good practice.
        ...
        if not documents_to_ingest:
            ...
            return # Exit ingest command gracefully if no documents are found

        logger.info(f"Successfully loaded {len(documents_to_ingest)} document(s). Proceeding with indexing into vector store.")
        app_vector_store.create_and_persist_index(documents_to_ingest) # Uses default storage path from app_config
    ...
    except ValueError as e_cli_val: # Catches API key errors from ensure_openai_api_key or other value errors
        logger.error(f"Ingestion failed due to a value or configuration error: {e_cli_val}")
        print(f"Error: {e_cli_val}")
    except Exception as e_cli_unexpected_ingest: # Catch-all for other unexpected errors during ingest
...
def handle_query_command(args: argparse.Namespace) -> None:
    ...
    try:
        app_config.ensure_openai_api_key() # Ensure API key is available for the query LLM

        query_engine_instance = app_query_engine.get_query_engine() # Attempts to load default index
    ...
    except ValueError as e_cli_val_query: # Catches API key errors or other value errors
        logger.error(f"Query failed due to a value or configuration error: {e_cli_val_query}")
        print(f"Error: {e_cli_val_query}")
    except Exception as e_cli_unexpected_query: # Catch-all for other unexpected errors during query
...
def main_application_cli() -> None: # Renamed to avoid conflict with any bash 'main' if wrapped
    ...
    parser = argparse.ArgumentParser(
        ...
        formatter_class=argparse.RawTextHelpFormatter # Allows for newlines in help text for better readability
    )
    subparsers = parser.add_subparsers(dest="command", help="Available commands: 'ingest' or 'query'", required=True)

    # --- Ingest command parser ---
    ...
    if len(sys.argv) == 1: # No command or arguments provided by the user
    ...
    if hasattr(parsed_args, 'func'):
        parsed_args.func(parsed_args) # Call the appropriate handler function (handle_ingest or handle_query)
    else: # Should not happen with 'required=True' for subparsers, but as a safeguard
