# Supagrok Tipi Service: Testing ionos_deploy_restricted.py on IONOS Server
## Ticket ID: SUPAGROK-IONOS-RESTRICTED-TEST-2025-05-06-001
## UUID: 7d6e5f4c-3b2a-1d0c-9e8f-7a6b5c4d3e2f

## Summary
This guide provides a comprehensive, one-shot procedure to test the `ionos_deploy_restricted.py` script on your IONOS server with limited permissions. The script is specifically designed to work in restricted environments where sudo access is unavailable, and it will automatically adapt its functionality based on the detected permissions and available tools.

## User Profile
- **User**: IONOS server user with limited permissions (e.g., a1678973)
- **Server**: IONOS VPS or Dedicated Server with restricted access
- **Access**: SSH access without sudo privileges

## Prerequisites
1. SSH access to your IONOS server
2. Python 3.6+ installed on your IONOS server
3. Ability to create files and directories in your home directory
4. pip or pip3 available for installing Python packages locally

## One-Shot Testing Instructions
...
```bash
mkdir -p ~/projects/supagrok-tipiservice/data/source ~/projects/supagrok-tipiservice/data/output
cd ~/projects/supagrok-tipiservice
```

### Step 2: Create the ionos_deploy_restricted.py Script

1. Create the script file:
```bash
touch ionos_deploy_restricted.py
chmod +x ionos_deploy_restricted.py
```

2. Open the file in your preferred editor (nano, vim, etc.):
```bash
nano ionos_deploy_restricted.py
```

3. Copy and paste the entire script content below into the editor:
